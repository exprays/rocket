...
const createCountryTable = async () => {
  try {
    ...
    throw new Error(`Error creating countries table: ${error.message}`);
  }
};

const insertCountry = async (country: string) => {
  try {
    // Insert country into the table
    await sql`INSERT INTO countries (country_name) VALUES (${country})`;
    console.log(`Inserted country: ${country}`);
  } catch (error: any) {
    throw new Error(`Error inserting country ${country}: ${error.message}`);
  }
};
